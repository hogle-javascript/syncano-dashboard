                  ...
                  <CardText>
                    ...
                    <CodePreview>
                      <CodePreview.Item
                        ...
                        code={`curl -X GET\n-H "X-API-KEY: ${token}"\n"https://api.syncano.io/v1.1/instances/${currentInstance.name}/schedules/${item.id}"`} />
                      <CodePreview.Item
                        title="Python"
                        languageClassName="python"
                        code={`schedule = Schedule.please.get(instance_name='${currentInstance.name}', id='${item.id}')\nprint(schedule.label)`} />
                        <CodePreview.Item
                        title="JavaScript"
                        languageClassName="javascript"
                        code={`Schedule\n  .please()\n  .get({instanceName: '${currentInstance.name}', id: '${item.id}'})\n  .then(function(schedules) {});`} />
                    </CodePreview>
                  </CardText>
                </Card>
              </div>
